"""Vertical and lateral movement: up, left, right, down.

The Tello SDK forbids `left` / `right` distances below 20 cm. If you go
lower the drone will just respond with `error` and ignore the command.
"""
import socket
import time
...
if __name__ == '__main__':
    ...
    if battery is None or battery < BATTERY_MIN:
        ...
        send_command(sock, 'left 80', tello_addr)
        send_command(sock, 'right 80', tello_addr)
        send_command(sock, 'down 40', tello_addr)
